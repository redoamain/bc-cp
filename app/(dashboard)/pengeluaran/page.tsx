...
export default function PengeluaranPage() {
  ...
  const [filteredData, setFilteredData] = useState<PengeluaranType[]>([]);
  ...
  const fetchData = useCallback(async (tglAwal: string, tglAkhir: string) => {
    ...
    try {
      ...
      const response = await getPengeluaran(tglAwal, tglAkhir);
  ...
  // Fungsi export dengan header laporan dan notifikasi Telegram
  const exportToExcel = async () => {
    try {
      ...
      // Hitung total per currency untuk notifikasi
      ...
      // Download file
      const fileName = `LAPORAN_PENGELUARAN_${tgl1}_${tgl2}.xlsx`;
