...
import { getBahanBaku } from "@/lib/services/mutasiService";
...
const sendTelegramNotification = async (exportData: {
  ...
  try {
    ...
    const response = await fetch("/api/notif", {
      ...
      body: JSON.stringify({
        message:
          `📦 |LAPORAN MUTASI BAHAN BAKU DIEXPORT\n\n` +
...
export default function BahanBakuPage() {
  ...
  const [hasData, setHasData] = useState(false); // Track apakah sudah pernah fetch data
  ...
  const fetchData = useCallback(async (tglAwal: string, tglAkhir: string) => {
    ...
    setHasData(false); // Reset hasData saat loading
    // Kosongkan data saat loading agar tidak tampil data lama
    // setData([]); // Opsional: jika ingin langsung kosongkan

    try {
      const response = await getBahanBaku(tglAwal, tglAkhir);
            ...
            <p className="text-muted-foreground">
              {isFirstLoad
                ...
                : hasData && data.length > 0
                  ? `Laporan mutasi bahan baku periode ${formatDate(tgl1)} - ${formatDate(tgl2)}`
                  : "Pilih periode dan klik tampilkan untuk melihat data"}
            </p>
          </div>
          ...
          <Card className="border-2 border-dashed border-muted-foreground/30">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-muted-foreground text-center max-w-md">
                ...
                untuk melihat laporan mutasi bahan baku.
              </p>
            </CardContent>
          </Card>
        ) : hasData && data.length > 0 ? (
