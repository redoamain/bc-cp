...
export default function BarangModalPage() {
  ...
  const exportToExcel = async () => {
    try {
      ...
      const fileName = `LAPORAN_BARANG_MODAL_${tgl1}_${tgl2}.xlsx`;
            ...
            <p className="text-muted-foreground">
              {isFirstLoad
                ...
                : hasData && data.length > 0
                  ? `Laporan mutasi barang modal periode ${formatDate(tgl1)} - ${formatDate(tgl2)}`
                  : "Pilih periode dan klik tampilkan untuk melihat data"}
            </p>
          </div>
          ...
          <Card className="border-2 border-dashed border-muted-foreground/30">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-muted-foreground text-center max-w-md">
                ...
                untuk melihat laporan mutasi barang modal.
              </p>
            </CardContent>
          </Card>
        ) : hasData && data.length > 0 ? (
