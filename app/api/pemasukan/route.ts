...
export async function GET(request: NextRequest) {
  try {
    ...
    const filteredData = result.recordset.filter((item) => {
      ...
      if (item.TanggalBPB) {
        const tglBPB = new Date(item.TanggalBPB);
    ...
    const transformedData = filteredData.map((item) => ({
      ...
      NomorBPB: item.NomorBPB ? Number(item.NomorBPB) : 0,
      TanggalBPB: item.TanggalBPB || null,
      PemasokPengirim: item.PemasokPengirim || "",
    ...
    return NextResponse.json({
      ...
      message: `Data pemasukan periode ${tgl1} s/d ${tgl2} berhasil diambil`,
      meta: {
