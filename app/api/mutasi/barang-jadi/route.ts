...
// app/api/mutasi/barang-jadi/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool
      ...
      .input("kategori", "BARANG JADI")
      .input("itemid", "%")
      .execute("rpMutasiBarangbc");

        const transformedData: MutasiType[] = result.recordset.map(
          (item: any) => ({
    ...
    return NextResponse.json({
      ...
      message: `Data barang jadi periode ${tgl1} s/d ${tgl2} berhasil diambil`,
      meta: {
        total: transformedData.length,
      },
    });
  } catch (error: any) {
    console.error("Error fetching barang jadi:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Gagal mengambil data barang jadi",
